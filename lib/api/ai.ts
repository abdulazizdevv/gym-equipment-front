...
export async function postAiEquipmentSearch(payload: { image: File; question?: string } & AiLangOptions) {
  ...
  const { data } = await http.post<AiPostEquipmentResponse>("/ai/equipment", form, {
    headers: { "Content-Type": "multipart/form-data", ...withLangHeaders(payload.lang) },
  });
  return data;
}

export async function postAiEquipmentFollowup(payload: { sessionId: number; question: string } & AiLangOptions) {
...
export async function getAiSessions(options?: AiListQueryOptions) {
  const { data } = await http.get<
    | AiSessionListItem[]
    | { items?: AiSessionListItem[]; data?: AiSessionListItem[]; meta?: AiSessionsMeta }
  >(
    "/ai/sessions",
    {
      headers: withLangHeaders(options?.lang),
...
export async function deleteAiSession(id: number, options?: AiLangOptions) {
  const { data } = await http.delete<{ message: string }>(`/ai/sessions/${id}`, {
    headers: withLangHeaders(options?.lang),
  });
  return data;
}
